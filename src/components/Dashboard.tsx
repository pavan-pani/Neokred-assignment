...
const Dashboard: React.FC = () => {
  ...
  const cardData =[
    { title: "Total revenue", amount: "$2.6M", percentageChange: "+4.5%", changeColor: "bg-[#60fe5169] text-[#6ffe32]" },
    { title: "Avarge order value", amount: "$446", percentageChange: "-0.5%", changeColor: "bg-[#fe515169] text-[#fe5732]" },
    { title: "Tickets sold", amount: "5,888", percentageChange: "+4.5%", changeColor: "bg-[#60fe5169] text-[#6ffe32]" },
    { title: "Pageviews", amount: "850,159", percentageChange: "+21%", changeColor: "bg-[#60fe5169] text-[#6ffe32]" },
  ]
      ...
      <h6 className="text-[#dcdcdc] mt-2 mb-5 ">Good Morning, Pavan</h6>
        ...
        <div className="flex justify-between py-3">
          ...
          <button className="p-1 flex justify-between gap-4 items-center px-3 text-center text-white rounded-md bg-[#494747]">
            Last week
            <span><HiOutlineChevronUp size={10}/><HiOutlineChevronDown size={10}/></span>
          </button>
        </div>

        <div className="flex gap-6 flex-wrap justify-between ">
        {cardData.map((card, index) => (
          <Suspense fallback={<div className='text-center text-xl'>Cards Loading...</div>}>
            <Card
              key={index}
              title={card.title}
              amount={card.amount}
